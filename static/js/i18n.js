// Diccionario y utilidades de internacionalización (i18n) para Cicada.
const I18N = {
    es: {
        nav_metadata: "Metadatos", nav_download: "Descarga", nav_playlist: "Playlist", nav_library: "Biblioteca", nav_ipod: "iPod",
        settings_tooltip: "Ajustes", connection_tooltip: "Estado de conexión",
        settings_title: "Ajustes",
        settings_tab_general: "General",
        settings_tab_apariencia: "Apariencia",
        settings_tab_integraciones: "Integraciones",
        settings_tab_ipod: "Módulo iPod",
        settings_external_services_title: "SERVICIOS EXTERNOS",
        settings_identification_behavior_title: "COMPORTAMIENTO DE IDENTIFICACIÓN",
        settings_theme_title: "TEMA",
        settings_theme_dark: "Grafito",
        settings_theme_light: "Aluminio",
        settings_color_title: "COLOR NANO",
        settings_language_title: "Idioma",
        settings_spotify_title: "Cuenta de Spotify",
        settings_spotify_connected: "Conectado a Spotify",
        settings_spotify_not_connected: "No conectado a Spotify",
        settings_spotify_connect_btn: "Conectar con Spotify",
        settings_spotify_reconnect_btn: "Reconectar con Spotify",
        settings_credentials_title: "Claves de Acceso",
        settings_credentials_help_tooltip: "¿Cómo obtener las claves?",
        settings_acoustid_label: "Clave de AcoustID",
        settings_spotify_id_label: "ID de Cliente de Spotify",
        settings_spotify_secret_label: "Clave Secreta de Spotify",
        settings_identification_title: "Identificación de Canciones",
        settings_plan_c_label: "Adivinar por el nombre del archivo cuando no se reconoce la canción",
        settings_plan_c_hint: "Apagado por defecto: suele ser poco preciso. Si está apagado, esos archivos se reportan como error en vez de adivinar el título/artista.",
        settings_ipod_section_title: "Módulo iPod",
        settings_ipod_ui_label: "Mostrar la sección iPod en la interfaz",
        settings_ipod_ui_hint: "Oculta la sección iPod de la interfaz. No reduce el tamaño de instalación ni elimina dependencias — el módulo sigue instalado, solo se deja de mostrar.",
        settings_accessibility_title: "Accesibilidad",
        settings_dyslexic_font_label: "Fuente para dislexia",
        settings_dyslexic_font_hint: "Reemplaza la tipografía de toda la interfaz por OpenDyslexic. Funciona sin recargar la página.",
        settings_colorblind_mode_label: "Modo daltónico",
        settings_colorblind_mode_hint: "Cambia los colores de estado (procesando/saltado/error/cancelado) a una paleta distinguible para daltonismo rojo-verde. Se puede usar junto a cualquier tema.",
        settings_folders_title: "Carpetas Predeterminadas",
        settings_library_dir_label: "Carpeta de tu Biblioteca",
        settings_input_dir_label: "Carpeta de Origen (Metadatos)",
        settings_output_dir_label: "Carpeta de Destino (Metadatos)",
        about_tooltip: "Sobre Cicada",
        about_version: "Versión {version}",
        about_description: "Herramienta local de organización musical y sincronización automática de metadatos de alta fidelidad.",
        about_author_label: "Desarrollado por",
        about_license: "Distribuido bajo Licencia GNU GPLv3",
        about_terms: "Términos y Condiciones",
        about_github_btn: "Ver en GitHub",
        about_contribute_btn: "Contribuir",
        server_status_title: "Estado del Servidor",
        server_status_checking: "Consultando...",
        server_status_connected: "Conectado",
        server_status_disconnected: "Desconectado",
        server_status_version: "Versión de Cicada",
        server_status_ipod_label: "iPod",
        server_status_ipod_connected: "Conectado",
        server_status_ipod_no_control: "Detectado, sin iPod_Control",
        server_status_ipod_none: "Sin dispositivo",
        kofi_support_title: "¡Gran trabajo!",
        kofi_support_message: "Has etiquetado {count} canciones en {time}. Hacer esto a mano te habría tomado {manual_time}. Apoya a mantener Cicada libre de anuncios.",
        update_available_text: "Nueva versión disponible: v{version}",
        update_available_link: "Ver última versión",
        common_choose: "Elegir", common_cancel: "Cancelar", common_save: "Guardar", common_done: "Listo",
        settings_saving: "Guardando...", settings_saved: "Guardado ✓",
        process_folders_title: "Carpetas de Trabajo",
        process_source_label: "Carpeta de Origen", process_dest_label: "Carpeta de Destino",
        process_start_btn: "Iniciar", process_cancel_btn: "Cancelar", process_cancel_full: "Cancelar Proceso",
        process_recent_activity_title: "Actividad Reciente",
        process_no_files_yet: "Todavía no se procesó ningún archivo en esta sesión.",
        process_view_more: "Ver más",
        process_progress_title: "Progreso", process_connection_title: "Conexión",
        process_activity_log_title: "Registro de Actividad",
        process_log_ready: "Listo. Esperando instrucciones...",
        process_connecting_btn: "Conectando...", process_cancelling_btn: "Cancelando...",
        process_waiting_first_file: "Esperando el primer archivo...",
        process_scanning_library: "Escaneando biblioteca",
        process_starting_status: "Iniciando", process_starting_track: "Iniciando...",
        process_track_of: "Pista {current} de {total}",
        process_skipped: "Saltado", process_processing: "Procesando",
        process_done_all: "Todas las pistas procesadas", process_stopped: "Proceso detenido",
        process_cancelled_status: "Cancelado", process_completed_status: "Completado",
        process_report_saved: "Reporte guardado en: ",
        ws_connected: "Conectado", ws_connecting_short: "Conectando", ws_connecting_dots: "Conectando...",
        ws_error: "Error", ws_disconnected: "Desconectado",
        log_ws_error: "Error en la conexión con el servidor (WebSocket desconectado).",
        log_ws_closed: "Conexión cerrada. Refresca la página para reconectar.",
        alert_both_paths_required: "Ambas rutas son requeridas.",
        log_starting_process: "Iniciando petición de procesamiento...",
        log_connect_error: "Error al conectar con el servidor: ",
        spotify_link_title: "Enlace de Música (Spotify / YouTube Music / Deezer)", spotify_analyze_btn: "Analizar",
        spotify_tracks_found_title: "Canciones Encontradas", spotify_select_all: "Seleccionar Todas",
        spotify_hint_paste_link: "Pega un link de Spotify, YouTube Music o Deezer (canción, álbum o playlist) y presiona Analizar para ver las canciones.",
        spotify_download_selected_btn: "Descargar Seleccionadas",
        alert_paste_link_first: "Pega un enlace de música primero.",
        spotify_analyzing_status: "Analizando enlace...", spotify_analyzing_btn: "Analizando...",
        ctx_show_in_folder: "Mostrar en la biblioteca",
        ctx_get_info: "Obtener información",
        ctx_delete_track: "Eliminar de biblioteca",
        ctx_ipod_add_to_playlist: "Agregar a Playlist",
        ctx_ipod_rate: "Calificar",
        ctx_ipod_remove: "Eliminar del iPod",
        ctx_ipod_no_playlists: "No hay playlists en el iPod",
        ctx_ipod_added_to_playlist: "✓ Agregada a \"{name}\".",
        ctx_ipod_already_in_playlist: "Esta canción ya está en \"{name}\".",
        ctx_ipod_remove_confirm: "¿Eliminar \"{title}\" del iPod? Esta acción no se puede deshacer.",
        track_info_title: "Información de la pista", track_info_tab_details: "Detalles", track_info_tab_artwork: "Carátula",
        track_info_change_artwork: "Cambiar Carátula",
        error_unknown: "Error desconocido", error_prefix: "Error: ",
        ipod_library_title: "Biblioteca del iPod",
        ipod_playlists_label: "Playlists", ipod_tracks_label: "Canciones",
        ipod_model_label: "Modelo", ipod_capacity_label: "Capacidad", ipod_format_label: "Formato", ipod_signature_label: "Firma",
        ipod_unknown: "Desconocido", ipod_loading: "Leyendo la biblioteca del iPod…",
        ipod_no_device: "No se ha detectado ningún iPod. Asegúrate de que esté conectado y montado.",
        ipod_no_control: "Se detectó un volumen sin estructura de iPod (¿recién formateado, o iPod_Control borrado?). No es una biblioteca de iPod utilizable.",
        ipod_scan_error: "Ocurrió un error al escanear dispositivos.",
        ipod_tracks_count: "canciones", ipod_playlists_count: "playlists",
        ipod_write_no_device: "No hay ningún iPod montado.",
        ipod_write_unsafe: "El iPod no está en estado seguro para escribir (identidad no verificada). Reconéctalo por USB e inténtalo de nuevo.",
        ipod_disconnected_mid_operation: "El iPod se desconectó durante la operación. Verifica la conexión e inténtalo de nuevo.",
        ipod_write_empty: "No hay pistas en el iPod que reescribir.",
        ipod_write_review: "Se reescribirá la base de datos del iPod con {n} pistas, en el formato de Cicada. Se hará un backup automático antes de escribir. ¿Continuar?",
        ipod_write_consent: "ADVERTENCIA\n\nCicada firma la base del iPod con una clave HASHAB propia, no la de Apple. El iPod la acepta sin problema, y por sí sola esta diferencia NO rompe la compatibilidad con Music.app (verificado). Pero si este iPod ya tiene residuo de otra herramienta (iOpenPod u otra) de una sesión previa, Music.app SÍ puede rechazarlo, sin relación con lo que escriba Cicada — en ese caso, 'cicada ipod clean-foreign' puede ayudar.\n\n¿Aceptas y continúas?",
        ipod_write_ok: "✓ Escritura completada. {n} pistas escritas.",
        ipod_write_failed: "✗ La escritura falló: ",
        ipod_write_rolled_back: " (se restauró el backup; el iPod quedó como antes).",
        ipod_backup_confirm: "¿Crear un backup (solo base de datos) del iPod?",
        ipod_backup_ok: "✓ Backup creado: {path} {size}",
        ipod_cat_songs: "Canciones",
        ipod_cat_sync: "Sincronizar con Cicada",
        ipod_sync_pending_title: "Pendientes de sincronizar",
        ipod_sync_pending_hint: "Elementos seleccionados que aún no se han enviado al iPod.",
        ipod_sync_now: "Sincronizar",
        ipod_sync_empty: "No hay nada pendiente. Agrega canciones desde tu Biblioteca o Playlists.",
        ipod_sync_remove: "Quitar del carrito",
        ipod_sync_playlists: "Playlists a crear",
        ipod_cat_conflicts: "Conflictos",
        ipod_conflicts_title: "Conflictos de calificación",
        ipod_conflicts_hint: "Cambiaron en Cicada y en el iPod desde la última sincronización — elige cuál vale.",
        ipod_conflicts_use_local_all: "Usar Local en todos",
        ipod_conflicts_use_device_all: "Usar Dispositivo en todos",
        ipod_conflicts_local_label: "Local",
        ipod_conflicts_device_label: "Dispositivo",
        ipod_conflicts_use_local: "Usar Local",
        ipod_conflicts_use_device: "Usar Dispositivo",
        ipod_conflicts_empty: "No hay conflictos pendientes.",
        ipod_conflicts_no_rating: "Sin calificación",
        ipod_conflicts_confirm_all_local: "¿Usar el valor local en los {n} conflictos pendientes? Esto escribirá en el iPod.",
        ipod_conflicts_confirm_all_device: "¿Usar el valor del dispositivo en los {n} conflictos pendientes?",
        ipod_play_title: "Reproducir desde tu biblioteca local",
        ipod_playlist_save_order: "Guardar cambios",
        ipod_playlist_order_saved: "✓ Cambios de la playlist guardados en el iPod.",
        ipod_playlist_add_songs: "Agregar canciones",
        ipod_playlist_add_search: "Buscar en la biblioteca...",
        ipod_playlist_add_empty: "La biblioteca local está vacía. Escanéala primero.",
        ipod_playlist_add_noresults: "Sin resultados.",
        ipod_playlist_new_track: "Pista nueva (se copiará al guardar)",
        ipod_play_not_in_library: "Esta canción no está en tu biblioteca local; no se puede reproducir.",
        ipod_sync_review: "Se copiarán y añadirán {n} canción(es) al iPod (con backup automático). ¿Continuar?",
        ipod_cat_playlists: "Playlist",
        ipod_cat_videos: "Videos",
        ipod_cat_podcasts: "Podcast",
        ipod_cat_audiobooks: "Audiolibros",
        ipod_cat_photos: "Fotos",
        ipod_photos_title: "Fotos del iPod",
        ipod_add_photos: "Agregar fotos",
        ipod_photos_summary: "Fotos en el iPod",
        ipod_photos_summary_bytes: "El iPod tiene fotos ({size}).",
        ipod_photos_readonly_hint: "Cicada no gestiona fotos del iPod — esto es solo informativo.",
        ipod_storage_used: "usados",
        ipod_storage_free: "libres",
        ipod_storage_audio: "Audio",
        ipod_storage_video: "Video",
        ipod_storage_photos: "Fotos",
        ipod_storage_other: "Otro",
        ipod_storage_free_label: "Libre",
        ipod_btn_scan: "Escanear",
        ipod_btn_eject: "Eyectar",
        ipod_btn_sync: "Reparar",
        ipod_btn_sync_title: "Reescribe la base de datos del iPod (preservando sus playlists). No agrega música nueva.",
        ipod_btn_backup: "Respaldos",
        ipod_search_placeholder: "Buscar en el iPod...",
        ipod_filter_label: "Filtros",
        ipod_filter_all: "Todos",
        ipod_filter_genre: "Género",
        ipod_filter_year: "Año",
        ipod_filter_artist: "Artista",
        ipod_filter_album: "Álbum",
        ipod_view_list: "Lista",
        ipod_view_grid: "Cuadros",
        ipod_create_playlist: "Crear Playlist",
        ipod_import_playlist: "Importar Playlist",
        ipod_subscribe_podcast: "Suscribir Podcast",
        ipod_add_audiobook: "Agregar Audiolibro",
        ipod_add_media: "Agregar",
        ipod_delete_item: "Eliminar",
        ipod_delete_confirm: "¿Deseas eliminar este elemento del iPod?",
        ipod_eject_confirm: "¿Deseas expulsar el iPod de forma segura?",
        ipod_eject_success: "iPod expulsado con éxito.",
        ipod_no_items: "No hay elementos en esta categoría.",
        ipod_no_results: "No se encontraron coincidencias para la búsqueda.",
        spotify_could_not_analyze: "No se pudo analizar el enlace.",
        spotify_no_tracks_found: "No se encontraron pistas en ese enlace.",
        track_untitled: "Sin título", track_unknown_artist: "Artista Desconocido", track_unknown_album: "Álbum Desconocido",
        alert_choose_dest_folder: "Elige una carpeta de destino.",
        alert_select_at_least_one_track: "Selecciona al menos una pista.",
        spotify_downloading_btn: "Descargando...",
        log_starting_spotify_download: "Iniciando descarga de {n} pista(s) de Spotify...",
        spotify_preparing_download: "Preparando descarga",
        spotify_waiting_first_track: "Esperando la primera pista...",
        playlists_my_playlists_title: "Mis Playlists", playlists_loading: "Cargando tus playlists...",
        playlists_liked_songs: "Canciones que te gustan",
        playlists_choose_hint: "Elige una playlist de la izquierda para ver sus canciones.",
        playlists_choose_title: "Elige una playlist",
        playlists_local_library_label: "Tu Biblioteca Local", playlists_replicate_btn: "Replicar Playlist",
        playlists_preview_title: "Vista Previa de la Playlist",
        playlists_preview_hint: "Elige una playlist y presiona Replicar Playlist para armar aquí la vista previa. Vas a poder arrastrar las canciones para reordenarlas y destildar las que no quieras incluir.",
        playlists_m3u8_name_placeholder: "Nombre de la playlist", playlists_generate_btn: "Generar Playlist",
        playlists_send_to_ipod: "Enviar a iPod",
        playlists_sent_to_ipod: "Playlist '{name}' ({n} canciones) agregada al carrito. Se creará en el iPod al sincronizar.",
        playlists_ipod_none: "No hay canciones encontradas para enviar al iPod.",
        playlists_no_playlists_found: "No se encontraron playlists en tu cuenta.",
        playlists_track_count_suffix: " pistas",
        playlists_loading_songs: "Cargando canciones...", playlists_no_songs: "Esta playlist no tiene canciones.",
        alert_choose_local_library_first: "Elige la carpeta de tu biblioteca local primero.",
        alert_playlist_no_songs_loaded: "Esta playlist no tiene canciones cargadas.",
        confirm_replicate: "Se van a buscar las {n} canciones de '{name}' en tu biblioteca local ({dir}) para armar una playlist .m3u8.\n\n¿Continuar?",
        playlists_searching_btn: "Buscando...",
        alert_error_searching_matches: "Error buscando coincidencias: ",
        playlists_not_found_suffix: " · no encontrada en tu biblioteca",
        playlists_summary: "{matched}/{total} encontradas · {included} incluidas",
        alert_error_associating_file: "Error asociando el archivo: ",
        confirm_manual_match: "Se van a reescribir los tags de:\n{path}\n\ncon los datos de '{artist} - {title}' (Spotify), y el archivo se va a reorganizar dentro de tu biblioteca.\n\n¿Continuar?",
        alert_no_songs_to_generate: "No hay canciones incluidas para generar la playlist.",
        playlists_generating_btn: "Generando...",
        alert_playlist_generated: "Playlist generada en: ",
        log_playlist_generated: "Playlist '{name}' generada en: {path}",
        alert_error_generating_playlist: "Error generando la playlist: ",
        default_playlist_name: "Mi Playlist", default_playlist_name_generic: "Playlist",
        library_my_library_title: "Mi Biblioteca", library_save_scan_btn: "Guardar y Buscar Canciones",
        library_group_by_label: "Agrupar por", library_group_all: "Todas", library_group_artist: "Artista",
        library_group_album: "Álbum", library_group_playlist: "Playlist",
        library_configure_hint: "Configura la carpeta de tu biblioteca arriba para verla aquí.",
        alert_choose_folder_first: "Elige una carpeta primero.",
        alert_error_saving_config: "Error guardando la configuración: ",
        library_scanning: "Escaneando biblioteca...", library_track_count_suffix: " canciones",
        library_add_to_ipod: "Agregar a iPod",
        library_add_selected: "Agregar ({n}) al carrito",
        library_select_all_group: "Seleccionar todo",
        library_select_none: "Selecciona al menos una canción.",
        library_added_to_ipod: "{n} canción(es) agregada(s) al carrito de sincronización.",
        library_no_songs_in_folder: "No se encontraron canciones en esa carpeta.",
        audiobooks_title: "Audiolibros", audiobooks_scan_btn: "Buscar Audiolibros",
        audiobooks_hint: "Elegí una carpeta para buscar archivos .m4b, .m4a, .mp3 o .aac.",
        podcasts_title: "Podcasts", podcasts_subscribe_btn: "Suscribirse",
        podcasts_no_subscriptions: "Todavía no hay podcasts suscriptos.",
        podcasts_pick_feed_hint: "Suscribite a un feed para ver sus episodios.",
        podcasts_no_episodes: "Este podcast no tiene episodios.",
        podcasts_download_btn: "Descargar",
        podcasts_status_downloading: "Descargando...",
        podcasts_status_downloaded: "Descargado",
        podcasts_status_on_ipod: "En el iPod",
        podcasts_status_failed: "Falló la descarga",
        library_no_playlist_group: "Sin playlist",
        library_search_placeholder: "Buscar por título, artista o álbum...",
        library_no_search_results: "No se encontraron canciones que coincidan con la búsqueda.",
        library_sort_alpha: "A-Z", library_sort_tooltip: "Orden alfabético",
        library_view_list: "Lista", library_view_grid: "Grilla",
        alert_error_saving_settings: "Error guardando ajustes: ",
        player_waiting_status: "En espera", player_cicada_label: "Cicada", player_no_cover: "Sin carátula",
        player_waiting_title: "En espera...", player_configure_source_hint: "Configura una fuente para comenzar",
        player_remaining_time_label: "Tiempo Restante", player_progress_label: "Avance",
        player_cancel_process_btn: "Cancelar Proceso", player_title: "Reproductor",
        player_nothing_playing: "Nada sonando", player_choose_song_hint: "Elige una canción de tu biblioteca",
        player_hide: "Ocultar reproductor", player_show: "Mostrar reproductor"
    },
    en: {
        nav_metadata: "Metadata", nav_download: "Download", nav_playlist: "Playlist", nav_library: "Library", nav_ipod: "iPod",
        settings_tooltip: "Settings", connection_tooltip: "Connection status",
        settings_title: "Settings",
        settings_tab_general: "General",
        settings_tab_apariencia: "Appearance",
        settings_tab_integraciones: "Integrations",
        settings_tab_ipod: "iPod Module",
        settings_external_services_title: "EXTERNAL SERVICES",
        settings_identification_behavior_title: "IDENTIFICATION BEHAVIOR",
        settings_theme_title: "THEME",
        settings_theme_dark: "Graphite",
        settings_theme_light: "Aluminum",
        settings_color_title: "NANO COLOR",
        settings_language_title: "Language",
        settings_spotify_title: "Spotify Account",
        settings_spotify_connected: "Connected to Spotify",
        settings_spotify_not_connected: "Not connected to Spotify",
        settings_spotify_connect_btn: "Connect with Spotify",
        settings_spotify_reconnect_btn: "Reconnect with Spotify",
        settings_credentials_title: "Access Keys",
        settings_credentials_help_tooltip: "How do I get these keys?",
        settings_acoustid_label: "AcoustID Key",
        settings_spotify_id_label: "Spotify Client ID",
        settings_spotify_secret_label: "Spotify Client Secret",
        settings_identification_title: "Song Identification",
        settings_plan_c_label: "Guess from the file name when a song isn't recognized",
        settings_plan_c_hint: "Off by default: it tends to be inaccurate. When off, those files are reported as errors instead of guessing the title/artist.",
        settings_ipod_section_title: "iPod Module",
        settings_ipod_ui_label: "Show the iPod section in the interface",
        settings_ipod_ui_hint: "Hides the iPod section from the interface. It does not reduce install size or remove dependencies — the module stays installed, it's just no longer shown.",
        settings_accessibility_title: "Accessibility",
        settings_dyslexic_font_label: "Dyslexia-friendly font",
        settings_dyslexic_font_hint: "Replaces the interface typography with OpenDyslexic. Works without reloading the page.",
        settings_colorblind_mode_label: "Colorblind mode",
        settings_colorblind_mode_hint: "Changes the status colors (processing/skipped/error/cancelled) to a palette distinguishable for red-green color blindness. Can be used with any theme.",
        settings_folders_title: "Default Folders",
        settings_library_dir_label: "Your Library Folder",
        settings_input_dir_label: "Source Folder (Metadata)",
        settings_output_dir_label: "Destination Folder (Metadata)",
        about_tooltip: "About Cicada",
        about_version: "Version {version}",
        about_description: "Local music organization tool with high-fidelity automatic metadata syncing.",
        about_author_label: "Developed by",
        about_license: "Distributed under the GNU GPLv3 License",
        about_terms: "Terms and Conditions",
        about_github_btn: "View on GitHub",
        about_contribute_btn: "Contribute",
        server_status_title: "Server Status",
        server_status_checking: "Checking...",
        server_status_connected: "Connected",
        server_status_disconnected: "Disconnected",
        server_status_version: "Cicada Version",
        server_status_ipod_label: "iPod",
        server_status_ipod_connected: "Connected",
        server_status_ipod_no_control: "Detected, no iPod_Control",
        server_status_ipod_none: "No device",
        kofi_support_title: "Great work!",
        kofi_support_message: "You've tagged {count} songs in {time}. Doing this by hand would have taken you {manual_time}. Support keeping Cicada ad-free.",
        update_available_text: "New version available: v{version}",
        update_available_link: "View latest release",
        common_choose: "Choose", common_cancel: "Cancel", common_save: "Save", common_done: "Done",
        settings_saving: "Saving...", settings_saved: "Saved ✓",
        process_folders_title: "Working Folders",
        process_source_label: "Source Folder", process_dest_label: "Destination Folder",
        process_start_btn: "Start", process_cancel_btn: "Cancel", process_cancel_full: "Cancel Process",
        process_recent_activity_title: "Recent Activity",
        process_no_files_yet: "No files have been processed in this session yet.",
        process_view_more: "View more",
        process_progress_title: "Progress", process_connection_title: "Connection",
        process_activity_log_title: "Activity Log",
        process_log_ready: "Ready. Waiting for instructions...",
        process_connecting_btn: "Connecting...", process_cancelling_btn: "Cancelling...",
        process_waiting_first_file: "Waiting for the first file...",
        process_scanning_library: "Scanning library",
        process_starting_status: "Starting", process_starting_track: "Starting...",
        process_track_of: "Track {current} of {total}",
        process_skipped: "Skipped", process_processing: "Processing",
        process_done_all: "All tracks processed", process_stopped: "Process stopped",
        process_cancelled_status: "Cancelled", process_completed_status: "Completed",
        process_report_saved: "Report saved at: ",
        ws_connected: "Connected", ws_connecting_short: "Connecting", ws_connecting_dots: "Connecting...",
        ws_error: "Error", ws_disconnected: "Disconnected",
        log_ws_error: "Error connecting to the server (WebSocket disconnected).",
        log_ws_closed: "Connection closed. Refresh the page to reconnect.",
        alert_both_paths_required: "Both paths are required.",
        log_starting_process: "Starting processing request...",
        log_connect_error: "Error connecting to the server: ",
        spotify_link_title: "Music Link (Spotify / YouTube Music / Deezer)", spotify_analyze_btn: "Analyze",
        spotify_tracks_found_title: "Tracks Found", spotify_select_all: "Select All",
        spotify_hint_paste_link: "Paste a Spotify, YouTube Music, or Deezer link (song, album, or playlist) and click Analyze to view tracks.",
        spotify_download_selected_btn: "Download Selected",
        alert_paste_link_first: "Please paste a music link first.",
        spotify_analyzing_status: "Analyzing link...", spotify_analyzing_btn: "Analyzing...",
        error_unknown: "Unknown error", error_prefix: "Error: ",
        ipod_library_title: "iPod Library",
        ipod_playlists_label: "Playlists", ipod_tracks_label: "Songs",
        ipod_model_label: "Model", ipod_capacity_label: "Capacity", ipod_format_label: "Format", ipod_signature_label: "Signature",
        ipod_unknown: "Unknown", ipod_loading: "Reading the iPod library…",
        ipod_no_device: "No iPod detected. Make sure it is connected and mounted.",
        ipod_no_control: "A volume was detected without iPod structure (freshly formatted, or iPod_Control deleted?). It is not a usable iPod library.",
        ipod_scan_error: "An error occurred while scanning devices.",
        ipod_tracks_count: "songs", ipod_playlists_count: "playlists",
        ipod_write_no_device: "No iPod is mounted.",
        ipod_write_unsafe: "The iPod is not in a safe state to write (identity unverified). Reconnect it over USB and try again.",
        ipod_disconnected_mid_operation: "The iPod disconnected during the operation. Check the connection and try again.",
        ipod_write_empty: "There are no tracks on the iPod to rewrite.",
        ipod_write_review: "The iPod database will be rewritten with {n} tracks, in Cicada's format. An automatic backup is made before writing. Continue?",
        ipod_write_consent: "WARNING\n\nCicada signs the iPod database with its own HASHAB key, not Apple's. The iPod accepts it fine, and on its own this difference does NOT break Music.app compatibility (verified). But if this iPod already carries leftovers from another tool (iOpenPod or otherwise) from a previous session, Music.app CAN reject it, regardless of what Cicada writes — if that happens, 'cicada ipod clean-foreign' may help.\n\nAccept and continue?",
        ipod_write_ok: "✓ Write complete. {n} tracks written.",
        ipod_write_failed: "✗ Write failed: ",
        ipod_write_rolled_back: " (the backup was restored; the iPod is unchanged).",
        ipod_backup_confirm: "Create a backup (database only) of the iPod?",
        ipod_backup_ok: "✓ Backup created: {path} {size}",
        ipod_cat_songs: "Songs",
        ipod_cat_sync: "Sync with Cicada",
        ipod_sync_pending_title: "Pending sync",
        ipod_sync_pending_hint: "Selected items not yet sent to the iPod.",
        ipod_sync_now: "Sync",
        ipod_sync_empty: "Nothing pending. Add songs from your Library or Playlists.",
        ipod_sync_remove: "Remove from cart",
        ipod_sync_playlists: "Playlists to create",
        ipod_cat_conflicts: "Conflicts",
        ipod_conflicts_title: "Rating conflicts",
        ipod_conflicts_hint: "Changed in Cicada and on the iPod since the last sync — pick which one wins.",
        ipod_conflicts_use_local_all: "Use Local for all",
        ipod_conflicts_use_device_all: "Use Device for all",
        ipod_conflicts_local_label: "Local",
        ipod_conflicts_device_label: "Device",
        ipod_conflicts_use_local: "Use Local",
        ipod_conflicts_use_device: "Use Device",
        ipod_conflicts_empty: "No pending conflicts.",
        ipod_conflicts_no_rating: "No rating",
        ipod_conflicts_confirm_all_local: "Use the local value for all {n} pending conflicts? This will write to the iPod.",
        ipod_conflicts_confirm_all_device: "Use the device value for all {n} pending conflicts?",
        ipod_play_title: "Play from your local library",
        ipod_playlist_save_order: "Save changes",
        ipod_playlist_order_saved: "✓ Playlist changes saved to the iPod.",
        ipod_playlist_add_songs: "Add songs",
        ipod_playlist_add_search: "Search your library...",
        ipod_playlist_add_empty: "Your local library is empty. Scan it first.",
        ipod_playlist_add_noresults: "No results.",
        ipod_playlist_new_track: "New track (will be copied on save)",
        ipod_play_not_in_library: "This song isn't in your local library, so it can't be played.",
        ipod_sync_review: "{n} song(s) will be copied and added to the iPod (with automatic backup). Continue?",
        ipod_cat_playlists: "Playlist",
        ipod_cat_videos: "Videos",
        ipod_cat_podcasts: "Podcast",
        ipod_cat_audiobooks: "Audiobooks",
        ipod_cat_photos: "Photos",
        ipod_photos_title: "iPod Photos",
        ipod_add_photos: "Add photos",
        ipod_photos_summary: "Photos on iPod",
        ipod_photos_summary_bytes: "The iPod has photos ({size}).",
        ipod_photos_readonly_hint: "Cicada doesn't manage the iPod's photos — this is informational only.",
        ipod_storage_used: "used",
        ipod_storage_free: "free",
        ipod_storage_audio: "Audio",
        ipod_storage_video: "Video",
        ipod_storage_photos: "Photos",
        ipod_storage_other: "Other",
        ipod_storage_free_label: "Free",
        ipod_btn_scan: "Scan",
        ipod_btn_eject: "Eject",
        ipod_btn_sync: "Repair",
        ipod_btn_sync_title: "Rewrites the iPod's database (preserving its playlists). Doesn't add new music.",
        ipod_btn_backup: "Backup",
        ipod_search_placeholder: "Search iPod...",
        ipod_filter_label: "Filters",
        ipod_filter_all: "All",
        ipod_filter_genre: "Genre",
        ipod_filter_year: "Year",
        ipod_filter_artist: "Artist",
        ipod_filter_album: "Album",
        ipod_view_list: "List",
        ipod_view_grid: "Grid",
        ipod_create_playlist: "Create Playlist",
        ipod_import_playlist: "Import Playlist",
        ipod_subscribe_podcast: "Subscribe Podcast",
        ipod_add_audiobook: "Add Audiobook",
        ipod_add_media: "Add",
        ipod_delete_item: "Delete",
        ipod_delete_confirm: "Do you want to delete this item from the iPod?",
        ipod_eject_confirm: "Do you want to safely eject the iPod?",
        ipod_eject_success: "iPod safely ejected.",
        ipod_no_items: "No items in this category.",
        ipod_no_results: "No matching items found.",
        spotify_could_not_analyze: "Couldn't analyze the link.",
        spotify_no_tracks_found: "No tracks found in that link.",
        track_untitled: "Untitled", track_unknown_artist: "Unknown Artist", track_unknown_album: "Unknown Album",
        alert_choose_dest_folder: "Choose a destination folder.",
        alert_select_at_least_one_track: "Select at least one track.",
        spotify_downloading_btn: "Downloading...",
        log_starting_spotify_download: "Starting download of {n} Spotify track(s)...",
        spotify_preparing_download: "Preparing download",
        spotify_waiting_first_track: "Waiting for the first track...",
        playlists_my_playlists_title: "My Playlists", playlists_loading: "Loading your playlists...",
        playlists_liked_songs: "Liked Songs",
        playlists_choose_hint: "Choose a playlist on the left to see its songs.",
        playlists_choose_title: "Choose a playlist",
        playlists_local_library_label: "Your Local Library", playlists_replicate_btn: "Replicate Playlist",
        playlists_preview_title: "Playlist Preview",
        playlists_preview_hint: "Choose a playlist and press Replicate Playlist to build the preview here. You'll be able to drag songs to reorder them and uncheck the ones you don't want to include.",
        playlists_m3u8_name_placeholder: "Playlist name", playlists_generate_btn: "Generate Playlist",
        playlists_send_to_ipod: "Send to iPod",
        playlists_sent_to_ipod: "Playlist '{name}' ({n} songs) added to the cart. It will be created on the iPod when you sync.",
        playlists_ipod_none: "No matched songs to send to the iPod.",
        playlists_no_playlists_found: "No playlists found in your account.",
        playlists_track_count_suffix: " tracks",
        playlists_loading_songs: "Loading songs...", playlists_no_songs: "This playlist has no songs.",
        alert_choose_local_library_first: "Choose your local library folder first.",
        alert_playlist_no_songs_loaded: "This playlist has no songs loaded.",
        confirm_replicate: "The {n} songs from '{name}' will be searched for in your local library ({dir}) to build an .m3u8 playlist.\n\nContinue?",
        playlists_searching_btn: "Searching...",
        alert_error_searching_matches: "Error searching for matches: ",
        playlists_not_found_suffix: " · not found in your library",
        playlists_summary: "{matched}/{total} found · {included} included",
        alert_error_associating_file: "Error associating the file: ",
        confirm_manual_match: "The tags of:\n{path}\n\nwill be rewritten with the data from '{artist} - {title}' (Spotify), and the file will be reorganized within your library.\n\nContinue?",
        alert_no_songs_to_generate: "There are no songs included to generate the playlist.",
        playlists_generating_btn: "Generating...",
        alert_playlist_generated: "Playlist generated at: ",
        log_playlist_generated: "Playlist '{name}' generated at: {path}",
        alert_error_generating_playlist: "Error generating the playlist: ",
        default_playlist_name: "My Playlist", default_playlist_name_generic: "Playlist",
        library_my_library_title: "My Library", library_save_scan_btn: "Save and Scan Songs",
        library_group_by_label: "Group by", library_group_all: "All", library_group_artist: "Artist",
        library_group_album: "Album", library_group_playlist: "Playlist",
        library_configure_hint: "Set your library folder above to see it here.",
        alert_choose_folder_first: "Choose a folder first.",
        alert_error_saving_config: "Error saving the configuration: ",
        library_scanning: "Scanning library...", library_track_count_suffix: " songs",
        library_add_to_ipod: "Add to iPod",
        library_add_selected: "Add ({n}) to cart",
        library_select_all_group: "Select all",
        library_select_none: "Select at least one song.",
        library_added_to_ipod: "{n} song(s) added to the sync cart.",
        library_no_songs_in_folder: "No songs found in that folder.",
        audiobooks_title: "Audiobooks", audiobooks_scan_btn: "Find Audiobooks",
        audiobooks_hint: "Choose a folder to search for .m4b, .m4a, .mp3, or .aac files.",
        podcasts_title: "Podcasts", podcasts_subscribe_btn: "Subscribe",
        podcasts_no_subscriptions: "No podcasts subscribed yet.",
        podcasts_pick_feed_hint: "Subscribe to a feed to see its episodes.",
        podcasts_no_episodes: "This podcast has no episodes.",
        podcasts_download_btn: "Download",
        podcasts_status_downloading: "Downloading...",
        podcasts_status_downloaded: "Downloaded",
        podcasts_status_on_ipod: "On iPod",
        podcasts_status_failed: "Download failed",
        library_no_playlist_group: "No playlist",
        library_search_placeholder: "Search by title, artist or album...",
        library_no_search_results: "No songs match your search.",
        library_sort_alpha: "A-Z", library_sort_tooltip: "Alphabetical order",
        library_view_list: "List", library_view_grid: "Grid",
        alert_error_saving_settings: "Error saving settings: ",
        player_waiting_status: "Waiting", player_cicada_label: "Cicada", player_no_cover: "No cover",
        player_waiting_title: "Waiting...", player_configure_source_hint: "Set up a source to get started",
        player_remaining_time_label: "Time Remaining", player_progress_label: "Progress",
        player_cancel_process_btn: "Cancel Process", player_title: "Player",
        player_nothing_playing: "Nothing playing", player_choose_song_hint: "Choose a song from your library",
        player_hide: "Hide player", player_show: "Show player"
    },
    ja: {
        nav_metadata: "メタデータ", nav_download: "ダウンロード", nav_playlist: "プレイリスト", nav_library: "ライブラリ", nav_ipod: "iPod",
        settings_tooltip: "設定", connection_tooltip: "接続状態",
        settings_title: "設定",
        settings_tab_general: "一般",
        settings_tab_apariencia: "外観",
        settings_tab_integraciones: "連携",
        settings_tab_ipod: "iPodモジュール",
        settings_external_services_title: "外部サービス",
        settings_identification_behavior_title: "識別の動作",
        settings_theme_title: "テーマ",
        settings_theme_dark: "グラファイト",
        settings_theme_light: "アルミニウム",
        settings_color_title: "ナノカラー",
        settings_language_title: "言語",
        settings_spotify_title: "Spotifyアカウント",
        settings_spotify_connected: "Spotifyに接続済み",
        settings_spotify_not_connected: "Spotifyに未接続",
        settings_spotify_connect_btn: "Spotifyで接続",
        settings_spotify_reconnect_btn: "Spotifyに再接続",
        settings_credentials_title: "アクセスキー",
        settings_credentials_help_tooltip: "キーの取得方法は?",
        settings_acoustid_label: "AcoustIDキー",
        settings_spotify_id_label: "SpotifyクライアントID",
        settings_spotify_secret_label: "Spotifyクライアントシークレット",
        settings_identification_title: "楽曲の識別",
        settings_plan_c_label: "認識できない場合はファイル名から推測する",
        settings_plan_c_hint: "デフォルトではオフです（精度が低いため）。オフの場合、認識できなかったファイルは推測せずエラーとして報告されます。",
        settings_ipod_section_title: "iPodモジュール",
        settings_ipod_ui_label: "インターフェースにiPodセクションを表示する",
        settings_ipod_ui_hint: "インターフェースからiPodセクションを非表示にします。インストールサイズの削減や依存関係の削除は行われません — モジュール自体はインストールされたままで、表示だけがオフになります。",
        settings_accessibility_title: "アクセシビリティ",
        settings_dyslexic_font_label: "ディスレクシア対応フォント",
        settings_dyslexic_font_hint: "インターフェース全体のフォントをOpenDyslexicに置き換えます。ページの再読み込みなしで機能します。",
        settings_colorblind_mode_label: "色覚多様性モード",
        settings_colorblind_mode_hint: "状態の色（処理中/スキップ/エラー/キャンセル）を、赤緑色覚異常の方でも見分けやすいパレットに変更します。どのテーマとも併用できます。",
        settings_folders_title: "デフォルトフォルダ",
        settings_library_dir_label: "ライブラリフォルダ",
        settings_input_dir_label: "入力フォルダ（メタデータ）",
        settings_output_dir_label: "出力フォルダ（メタデータ）",
        about_tooltip: "Cicadaについて",
        about_version: "バージョン {version}",
        about_description: "高精度なメタデータの自動同期を行う、ローカル音楽整理ツール。",
        about_author_label: "開発者:",
        about_license: "GNU GPLv3ライセンスの下で配布",
        about_terms: "利用規約",
        about_github_btn: "GitHubで見る",
        about_contribute_btn: "支援する",
        server_status_title: "サーバーの状態",
        server_status_checking: "確認中...",
        server_status_connected: "接続済み",
        server_status_disconnected: "切断",
        server_status_version: "Cicadaのバージョン",
        server_status_ipod_label: "iPod",
        server_status_ipod_connected: "接続済み",
        server_status_ipod_no_control: "検出済み、iPod_Controlなし",
        server_status_ipod_none: "デバイスなし",
        kofi_support_title: "お疲れ様でした!",
        kofi_support_message: "{count}曲にタグ付けし、所要時間は{time}でした。手作業で行うと{manual_time}かかっていたはずです。Cicadaを広告なしで維持するためにご支援ください。",
        update_available_text: "新しいバージョンがあります: v{version}",
        update_available_link: "最新リリースを見る",
        common_choose: "選択", common_cancel: "キャンセル", common_save: "保存", common_done: "完了",
        settings_saving: "保存中...", settings_saved: "保存しました ✓",
        process_folders_title: "作業フォルダ",
        process_source_label: "入力フォルダ", process_dest_label: "出力フォルダ",
        process_start_btn: "開始", process_cancel_btn: "キャンセル", process_cancel_full: "処理をキャンセル",
        process_recent_activity_title: "最近のアクティビティ",
        process_no_files_yet: "このセッションではまだファイルが処理されていません。",
        process_view_more: "もっと見る",
        process_progress_title: "進捗", process_connection_title: "接続",
        process_activity_log_title: "アクティビティログ",
        process_log_ready: "準備完了。指示を待機中...",
        process_connecting_btn: "接続中...", process_cancelling_btn: "キャンセル中...",
        process_waiting_first_file: "最初のファイルを待機中...",
        process_scanning_library: "ライブラリをスキャン中",
        process_starting_status: "開始中", process_starting_track: "開始中...",
        process_track_of: "{total}中{current}曲目",
        process_skipped: "スキップ", process_processing: "処理中",
        process_done_all: "すべての曲を処理しました", process_stopped: "処理を停止しました",
        process_cancelled_status: "キャンセル済み", process_completed_status: "完了",
        process_report_saved: "レポートの保存先: ",
        ws_connected: "接続済み", ws_connecting_short: "接続中", ws_connecting_dots: "接続中...",
        ws_error: "エラー", ws_disconnected: "切断されました",
        log_ws_error: "サーバーとの接続エラー（WebSocket切断）。",
        log_ws_closed: "接続が閉じられました。ページを更新して再接続してください。",
        alert_both_paths_required: "両方のパスが必要です。",
        log_starting_process: "処理リクエストを開始しています...",
        log_connect_error: "サーバーへの接続エラー: ",
        spotify_link_title: "音楽リンク (Spotify / YouTube Music / Deezer)", spotify_analyze_btn: "分析",
        spotify_tracks_found_title: "見つかった曲", spotify_select_all: "すべて選択",
        spotify_hint_paste_link: "Spotify、YouTube Music、またはDeezerのリンク（曲、アルバム、プレイリスト）を貼り付けて「分析」をクリックしてください。",
        spotify_download_selected_btn: "選択した曲をダウンロード",
        alert_paste_link_first: "最初に音楽リンクを貼り付けてください。",
        spotify_analyzing_status: "リンクを分析中...", spotify_analyzing_btn: "分析中...",
        error_unknown: "不明なエラー", error_prefix: "エラー: ",
        ipod_library_title: "iPodのライブラリ",
        ipod_playlists_label: "プレイリスト", ipod_tracks_label: "曲",
        ipod_model_label: "モデル", ipod_capacity_label: "容量", ipod_format_label: "フォーマット", ipod_signature_label: "署名",
        ipod_unknown: "不明", ipod_loading: "iPodのライブラリを読み込み中…",
        ipod_no_device: "iPodが検出されません。接続とマウントを確認してください。",
        ipod_no_control: "iPod構造のないボリュームが検出されました（フォーマット直後、またはiPod_Controlが削除された？）。使用可能なiPodライブラリではありません。",
        ipod_scan_error: "デバイスのスキャン中にエラーが発生しました。",
        ipod_tracks_count: "曲", ipod_playlists_count: "プレイリスト",
        ipod_write_no_device: "iPodがマウントされていません。",
        ipod_write_unsafe: "iPodは書き込み可能な安全な状態ではありません（identityが未確認）。USBで再接続してからやり直してください。",
        ipod_disconnected_mid_operation: "操作中にiPodが切断されました。接続を確認してもう一度お試しください。",
        ipod_write_empty: "書き換える曲がiPodにありません。",
        ipod_write_review: "iPodのデータベースを{n}曲でCicada形式に書き換えます。書き込み前に自動バックアップを作成します。続行しますか？",
        ipod_write_consent: "警告\n\nCicadaはiPodのデータベースに独自のHASHAB署名を使用します(Appleの署名ではありません)。iPod本体はこれを問題なく受け入れ、この違いだけではMusic.appとの互換性は壊れません(確認済み)。ただし、このiPodに他のツール(iOpenPodなど)の以前のセッションの残留物がある場合、Cicadaの書き込み内容に関係なくMusic.appがデバイスを拒否することがあります — その場合は「cicada ipod clean-foreign」が役立つことがあります。\n\n同意して続行しますか？",
        ipod_write_ok: "✓ 書き込み完了。{n}曲を書き込みました。",
        ipod_write_failed: "✗ 書き込みに失敗しました： ",
        ipod_write_rolled_back: "（バックアップを復元しました。iPodは変更されていません）。",
        ipod_backup_confirm: "iPodのバックアップ（データベースのみ）を作成しますか？",
        ipod_backup_ok: "✓ バックアップを作成しました： {path} {size}",
        ipod_cat_songs: "曲",
        ipod_cat_sync: "Cicadaと同期",
        ipod_sync_pending_title: "同期待ち",
        ipod_sync_pending_hint: "まだiPodに送信していない選択項目。",
        ipod_sync_now: "同期",
        ipod_sync_empty: "保留中の項目はありません。ライブラリやプレイリストから曲を追加してください。",
        ipod_sync_remove: "カートから削除",
        ipod_sync_playlists: "作成するプレイリスト",
        ipod_cat_conflicts: "競合",
        ipod_conflicts_title: "評価の競合",
        ipod_conflicts_hint: "前回の同期以降、Cicadaとipodの両方で変更されました — どちらを採用するか選んでください。",
        ipod_conflicts_use_local_all: "すべてローカルを使用",
        ipod_conflicts_use_device_all: "すべてデバイスを使用",
        ipod_conflicts_local_label: "ローカル",
        ipod_conflicts_device_label: "デバイス",
        ipod_conflicts_use_local: "ローカルを使用",
        ipod_conflicts_use_device: "デバイスを使用",
        ipod_conflicts_empty: "保留中の競合はありません。",
        ipod_conflicts_no_rating: "評価なし",
        ipod_conflicts_confirm_all_local: "保留中の{n}件すべてにローカルの値を使用しますか？iPodに書き込まれます。",
        ipod_conflicts_confirm_all_device: "保留中の{n}件すべてにデバイスの値を使用しますか？",
        ipod_play_title: "ローカルライブラリから再生",
        ipod_playlist_save_order: "変更を保存",
        ipod_playlist_order_saved: "✓ プレイリストの変更をiPodに保存しました。",
        ipod_playlist_add_songs: "曲を追加",
        ipod_playlist_add_search: "ライブラリを検索...",
        ipod_playlist_add_empty: "ローカルライブラリが空です。先にスキャンしてください。",
        ipod_playlist_add_noresults: "結果なし。",
        ipod_playlist_new_track: "新しいトラック（保存時にコピーされます）",
        ipod_play_not_in_library: "この曲はローカルライブラリにないため再生できません。",
        ipod_sync_review: "{n}曲をiPodにコピーして追加します（自動バックアップ付き）。続行しますか？",
        ipod_cat_playlists: "プレイリスト",
        ipod_cat_videos: "ビデオ",
        ipod_cat_podcasts: "ポッドキャスト",
        ipod_cat_audiobooks: "オーディオブック",
        ipod_cat_photos: "写真",
        ipod_photos_title: "iPod 写真",
        ipod_add_photos: "写真を追加",
        ipod_photos_summary: "iPodの写真",
        ipod_photos_summary_bytes: "iPodに写真があります（{size}）。",
        ipod_photos_readonly_hint: "Cicadaは iPod の写真を管理しません — これは情報表示のみです。",
        ipod_storage_used: "使用済み",
        ipod_storage_free: "空き",
        ipod_storage_audio: "オーディオ",
        ipod_storage_video: "ビデオ",
        ipod_storage_photos: "写真",
        ipod_storage_other: "その他",
        ipod_storage_free_label: "空き容量",
        ipod_btn_scan: "スキャン",
        ipod_btn_eject: "取り出す",
        ipod_btn_sync: "修復",
        ipod_btn_sync_title: "iPodのデータベースを書き直します（プレイリストは保持）。新しい曲は追加されません。",
        ipod_btn_backup: "バックアップ",
        ipod_search_placeholder: "iPod内を検索...",
        ipod_filter_label: "フィルター",
        ipod_filter_all: "すべて",
        ipod_filter_genre: "ジャンル",
        ipod_filter_year: "年",
        ipod_filter_artist: "アーティスト",
        ipod_filter_album: "アルバム",
        ipod_view_list: "リスト",
        ipod_view_grid: "グリッド",
        ipod_create_playlist: "プレイリスト作成",
        ipod_import_playlist: "プレイリストをインポート",
        ipod_subscribe_podcast: "ポッドキャストを購読",
        ipod_add_audiobook: "オーディオブックを追加",
        ipod_add_media: "追加",
        ipod_delete_item: "削除",
        ipod_delete_confirm: "この項目をiPodから削除しますか？",
        ipod_eject_confirm: "iPodを安全に取り出しますか？",
        ipod_eject_success: "iPodは正常に取り出されました。",
        ipod_no_items: "このカテゴリには項目がありません。",
        ipod_no_results: "一致する項目が見つかりませんでした。",
        spotify_could_not_analyze: "リンクを分析できませんでした。",
        spotify_no_tracks_found: "このリンクには楽曲が見つかりませんでした。",
        track_untitled: "タイトルなし", track_unknown_artist: "不明のアーティスト", track_unknown_album: "不明のアルバム",
        alert_choose_dest_folder: "保存先フォルダを選択してください。",
        alert_select_at_least_one_track: "少なくとも1曲を選択してください。",
        spotify_downloading_btn: "ダウンロード中...",
        log_starting_spotify_download: "Spotifyの{n}曲のダウンロードを開始しています...",
        spotify_preparing_download: "ダウンロードを準備中",
        spotify_waiting_first_track: "最初の曲を待機中...",
        playlists_my_playlists_title: "マイプレイリスト", playlists_loading: "プレイリストを読み込み中...",
        playlists_liked_songs: "お気に入りの曲",
        playlists_choose_hint: "左のリストからプレイリストを選んで曲を表示します。",
        playlists_choose_title: "プレイリストを選択",
        playlists_local_library_label: "ローカルライブラリ", playlists_replicate_btn: "プレイリストを複製",
        playlists_preview_title: "プレイリストプレビュー",
        playlists_preview_hint: "プレイリストを選んで「プレイリストを複製」を押すと、ここにプレビューが作成されます。曲をドラッグして並び替えたり、不要な曲のチェックを外したりできます。",
        playlists_m3u8_name_placeholder: "プレイリスト名", playlists_generate_btn: "プレイリストを作成",
        playlists_send_to_ipod: "iPodに送信",
        playlists_sent_to_ipod: "プレイリスト「{name}」（{n}曲）をカートに追加しました。同期時にiPodで作成されます。",
        playlists_ipod_none: "iPodに送信できる一致した曲がありません。",
        playlists_no_playlists_found: "あなたのアカウントにプレイリストが見つかりませんでした。",
        playlists_track_count_suffix: " 曲",
        playlists_loading_songs: "楽曲を読み込み中...", playlists_no_songs: "このプレイリストには楽曲がありません。",
        alert_choose_local_library_first: "まずローカルライブラリのフォルダを選択してください。",
        alert_playlist_no_songs_loaded: "このプレイリストには読み込まれた楽曲がありません。",
        confirm_replicate: "'{name}'の{n}曲をローカルライブラリ（{dir}）から検索して.m3u8プレイリストを作成します。\n\n続けますか？",
        playlists_searching_btn: "検索中...",
        alert_error_searching_matches: "一致する曲の検索中にエラーが発生しました: ",
        playlists_not_found_suffix: " ・ ライブラリ内に見つかりません",
        playlists_summary: "{matched}/{total} 件一致 ・ {included} 件を含む",
        alert_error_associating_file: "ファイルの関連付け中にエラーが発生しました: ",
        confirm_manual_match: "次のファイルのタグを書き換えます:\n{path}\n\nSpotifyの'{artist} - {title}'のデータで上書きし、ファイルはライブラリ内で整理されます。\n\n続けますか？",
        alert_no_songs_to_generate: "プレイリストを作成するための曲が含まれていません。",
        playlists_generating_btn: "作成中...",
        alert_playlist_generated: "プレイリストを作成しました: ",
        log_playlist_generated: "プレイリスト「{name}」を作成しました: {path}",
        alert_error_generating_playlist: "プレイリストの作成中にエラーが発生しました: ",
        default_playlist_name: "マイプレイリスト", default_playlist_name_generic: "プレイリスト",
        library_my_library_title: "マイライブラリ", library_save_scan_btn: "保存して曲を検索",
        library_group_by_label: "グループ化", library_group_all: "すべて", library_group_artist: "アーティスト",
        library_group_album: "アルバム", library_group_playlist: "プレイリスト",
        library_configure_hint: "上のライブラリフォルダを設定するとここに表示されます。",
        alert_choose_folder_first: "まずフォルダを選択してください。",
        alert_error_saving_config: "設定の保存中にエラーが発生しました: ",
        library_scanning: "ライブラリをスキャン中...", library_track_count_suffix: " 曲",
        library_add_to_ipod: "iPodに追加",
        library_add_selected: "カートに追加 ({n})",
        library_select_all_group: "すべて選択",
        library_select_none: "少なくとも1曲を選択してください。",
        library_added_to_ipod: "{n}曲を同期カートに追加しました。",
        library_no_songs_in_folder: "このフォルダには楽曲が見つかりませんでした。",
        audiobooks_title: "オーディオブック", audiobooks_scan_btn: "オーディオブックを検索",
        audiobooks_hint: ".m4b、.m4a、.mp3、.aacファイルを検索するフォルダを選択してください。",
        podcasts_title: "ポッドキャスト", podcasts_subscribe_btn: "購読",
        podcasts_no_subscriptions: "まだ購読しているポッドキャストはありません。",
        podcasts_pick_feed_hint: "フィードを購読するとエピソードが表示されます。",
        podcasts_no_episodes: "このポッドキャストにはエピソードがありません。",
        podcasts_download_btn: "ダウンロード",
        podcasts_status_downloading: "ダウンロード中...",
        podcasts_status_downloaded: "ダウンロード済み",
        podcasts_status_on_ipod: "iPodに追加済み",
        podcasts_status_failed: "ダウンロード失敗",
        library_no_playlist_group: "プレイリストなし",
        library_search_placeholder: "タイトル、アーティスト、アルバムで検索...",
        library_no_search_results: "検索に一致する曲が見つかりませんでした。",
        library_sort_alpha: "A-Z", library_sort_tooltip: "アルファベット順",
        library_view_list: "リスト", library_view_grid: "グリッド",
        alert_error_saving_settings: "設定の保存中にエラーが発生しました: ",
        player_waiting_status: "待機中", player_cicada_label: "Cicada", player_no_cover: "カバーなし",
        player_waiting_title: "待機中...", player_configure_source_hint: "ソースを設定して開始してください",
        player_remaining_time_label: "残り時間", player_progress_label: "進捗",
        player_cancel_process_btn: "処理をキャンセル", player_title: "プレーヤー",
        player_nothing_playing: "再生中の曲はありません", player_choose_song_hint: "ライブラリから曲を選択してください",
        player_hide: "プレーヤーを非表示", player_show: "プレーヤーを表示"
    }
};
