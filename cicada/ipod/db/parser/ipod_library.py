...
Usage::
    ...
    from cicada.ipod.db.parser.ipod_library import load_ipod_library
...
from cicada.ipod.db.shared.field_base import filetype_to_string
...
def load_ipod_library(itunesdb_path: str,
    ...
    Args:
        ...
        mount: Optional explicit mount root (the directory that contains
            ``iPod_Control/``). When given it is used for the device-time
            context instead of deriving it as ``dirname³(itunesdb_path)``,
            which resolves wrongly (silently) for an iTunesCDB extracted
            outside the standard layout (e.g. from a backup). Cicada addition.
    ...
    try:
        ...
        mount_root = mount if mount is not None else os.path.dirname(
            os.path.dirname(os.path.dirname(itunesdb_path))
        )
