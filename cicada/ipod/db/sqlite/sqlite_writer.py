...
Usage:
    from cicada.ipod.db.sqlite import write_sqlite_databases
...
# Cicada: el stack de ESCRITURA-AL-DISPOSITIVO de iOpenPod (path_safety,
# write_readiness, su write_guard) NO se vendoriza — Cicada reimplementa la
# instalación (build_sqlite_databases → staging; install del coordinador vía
# safe_write). Estos imports se toleran; las vías directas no se llaman.
try:  # pragma: no cover - vías de escritura directa no usadas por Cicada
    from iopenpod.device import detect_checksum_type, get_firewire_id
except ImportError:
    detect_checksum_type = get_firewire_id = None  # type: ignore[assignment]
try:  # pragma: no cover
    ...
    resolve_device_path = DeviceWriteGuard = None  # type: ignore
    inspect_device_write_readiness = revalidate_device_write_readiness = None  # type: ignore
    volume_lock_key = None  # type: ignore
