...
from cicada.ipod.device.durability import (
    durable_replace,
    durable_unlink,
    flush_written_file,
...
# Cicada: el stack de ESCRITURA-AL-DISPOSITIVO de iOpenPod (filesystem, path_safety,
# storage_safety, write_readiness, su write_guard) NO se vendoriza — Cicada
# reimplementa el coordinador y usa el BUILDER puro `write_mhbd` + su propio
# comprimir/firmar/instalar (vía safe_write). Estos imports se toleran para que el
# módulo cargue; las vías de escritura directa (que Cicada no llama) degradan a None.
try:  # pragma: no cover - vías de escritura directa no usadas por Cicada
    from iopenpod.device import detect_checksum_type
except ImportError:
    detect_checksum_type = None  # type: ignore[assignment]
try:  # pragma: no cover
    from iopenpod.device.filesystem import detect_filesystem_type, resolve_itunesdb_platform
    from iopenpod.device.path_safety import resolve_device_path
    from iopenpod.device.storage_safety import (
        FileSizeLimitError, allocated_size, effective_max_file_size_bytes,
    ...
    detect_filesystem_type = resolve_itunesdb_platform = resolve_device_path = None  # type: ignore
    FileSizeLimitError = allocated_size = effective_max_file_size_bytes = None  # type: ignore
    require_file_size_supported = inspect_device_write_readiness = None  # type: ignore
