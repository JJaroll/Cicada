...
* **🎧 iPod連携:** iPodを検出し、音楽とプレイリストを同期（評価の競合を解決する双方向同期を含む）、カバーアートを書き込み、動画・ポッドキャスト・オーディオブックを管理します — すべて自動バックアップとエラー時のロールバック付き。詳細は下記の専用セクションをご覧ください。
...
* **音楽とプレイリスト:** ライブラリの読み書き、Cicadaからデバイスへの
  プレイリストの作成と送信。
...
* **写真:** 徹底的に調査しました（実際のiTunes/Music同期やサードパーティ
  ツールとの比較を含む、7つの独立した調査ライン）が、ファームウェアを
  分解せずには根本原因を特定できず、最終的にプロジェクトから除外され
  ました。詳細は[`docs/VENDORED.md`](docs/VENDORED.md)（パッケージ9）
  および[`docs/IPOD_INTEGRATION.md`](docs/IPOD_INTEGRATION.md)を参照
  してください。
