...
def test_backup_db_only_no_incluye_music(ipod, backups_dir):
    ...
    assert not any("Music" in n for n in names)
...
def test_backup_excluye_artefactos_macos(ipod, backups_dir):
    ...
    assert not any(".Trashes" in n for n in names)
...
def test_rotacion_no_afecta_full(ipod, backups_dir):
    ...
    guid_dir = backups_dir / UNKNOWN_GUID
    assert len(list(guid_dir.glob("*_full.tar.zst"))) == 3
    assert len(list(guid_dir.glob("*_db-only.tar.zst"))) == 20
...
def test_round_trip_db_only(ipod, backups_dir):
    ...
    (control / "iTunes" / "iTunesCDB").write_bytes(b"CDB-CORROMPIDO")
    (control / "iTunes" / "iTunes Library.itlp" / "Library.itdb").unlink()
    (control / "iTunes" / "playlist-fantasma.itdb").write_bytes(b"basura")
...
def test_round_trip_full(ipod, backups_dir):
    ...
    assert not (control / "Music" / "F05").exists()
...
def test_photos_protegida_de_rmtree_completo_durante_restore(ipod_with_photos, backups_dir, monkeypatch):
    photos_root = (ipod_with_photos / "Photos").resolve()
    import shutil
    real_rmtree = shutil.rmtree
    def guarded_rmtree(path, *a, **k):
        assert os.path.realpath(path) != str(photos_root), \
            f"restore intentó rmtree de Photos/ completa: {path}"
        return real_rmtree(path, *a, **k)
    monkeypatch.setattr(shutil, "rmtree", guarded_rmtree)
